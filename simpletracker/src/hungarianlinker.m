...
% implementing the Munkres algorithm; this file is merely a wrapper for it.
    ...
    %% added by Julien Dubrulle
    [r,c]=find(target_indices);
    ti=-1*ones(size(target_indices,1),1);
    ti(r)=c;
    target_indices=ti;
    % Set unmatched sources to -1
    %target_indices = double(target_indices); % added by Julien or else line 104 would bug
